...
/// Build an AviSynth script that reproduces the ffmpeg preprocessing for
/// `-rs`/`-lb`/`-pb`/`-sf` and HDR→SDR: the source is loaded via
/// `DirectShowSource` (AviSynth built-in), retimed with an exact
/// `AssumeFPS(num, den)` (timestamps only, no frame add/drop — including the
/// automatic VFR→CFR normalisation), scaled (`LanczosResize`) and padded with
/// black bars (`AddBorders`). HDR sources (PQ/HLG) get a tone-mapped
/// BT.2020→BT.709 chain (`z_ConvertFormat` linearisation + `DGHable` tone
/// map, avsresize/DGTonemap); the result is output as YUV 4:2:0
/// (`ConvertToYV12`).
/// AviSynth has no SAR concept, so pixels are square (SAR 1:1) by default.
fn build_avs_script(
    input: &str,
    target: Option<RescaleTarget>,
    letterbox: bool,
    pillarbox: bool,
    retime: Option<FrameRate>,
    info: &VideoInfo,
) -> String {
    let transfer = info.color_transfer.as_deref();
    let is_hdr = transfer.is_some_and(is_hdr_transfer);
    ...
    lines.push("# AviSynth script generated by xwaf".to_string());
    lines.push(format!("# input: {input}"));
    lines.push(String::new());
    if is_hdr {
        lines.push("# HDR sources require: avsresize (z_ConvertFormat) + DGTonemap (DGHable)".to_string());
    }
    lines.push(String::new());
    // Escape backslashes for the AviSynth string literal.
    let path = input.replace('\\', "\\\\");
    lines.push(format!("DirectShowSource(\"{path}\", audio=false)"));
    lines.push(String::new());

    if let Some(r) = retime {
        lines.push(
            "# --setfps / automatic CFR: exact rational retime without adding or dropping frames"
                .to_string(),
        );
        lines.push(format!("AssumeFPS({}, {})", r.num, r.den));
        lines.push(String::new());
    }
    ...
    if let Some(t) = target {
        let (w, h) = t.size();
        let canvas_aspect = w as f64 / h as f64;
        let source_wider = info.display_aspect > canvas_aspect;
        let pad_requested = if source_wider { letterbox } else { pillarbox };
        let (ow, oh) = if source_wider {
            (w as i64, even((w as f64 / info.display_aspect).round() as i64))
        } else {
            (even((h as f64 * info.display_aspect).round() as i64), h as i64)
        };
        if ow as u32 != info.width || oh as u32 != info.height {
            lines.push("# --rescale: Lanczos scaling".to_string());
            lines.push(format!("LanczosResize({ow}, {oh})"));
            lines.push(String::new());
        }
        let leftover = if source_wider {
            oh < h as i64
        ...
        if pad_requested && leftover {
            let extra_w = (w - ow as u32) as i64;
            let extra_h = (h - oh as u32) as i64;
            let (left, top, right, bottom) = if source_wider {
                (0, extra_h / 2, 0, extra_h - extra_h / 2)
            } else {
                (extra_w / 2, 0, extra_w - extra_w / 2, 0)
            };
            lines.push("# --letterbox/--pillarbox: black bars".to_string());
            lines.push(format!("AddBorders({left}, {top}, {right}, {bottom})"));
            lines.push(String::new());
        }
    }

    if is_hdr {
        // Tone map HDR → SDR: linearise the PQ/HLG signal with z_ConvertFormat
        // (avsresize/zimg, the same family as ffmpeg's zscale), compress it
        // with DGHable (DGTonemap), then convert BT.2020 primaries → BT.709.
        let tin = if transfer.is_some_and(|t| t.eq_ignore_ascii_case("arib-std-b67")) {
            "std-b67"
        } else {
            "st2084"
        };
        lines.push("# HDR→SDR: linearise (PQ/HLG), Hable tone map (DGHable), BT.2020 → BT.709".to_string());
        lines.push(format!(
            "z_ConvertFormat(pixel_type=\"RGBPS\", colorspace_op=\"2020ncl:{tin}:2020:l=>rgb:linear:2020:l\", dither_type=\"none\")"
        ));
        lines.push("DGHable()".to_string());
        lines.push("z_ConvertFormat(pixel_type=\"YV12\", colorspace_op=\"rgb:linear:2020:l=>709:709:709:l\", dither_type=\"ordered\")".to_string());
        lines.push(String::new());
    }

    lines.push("# Square pixels (SAR 1:1) — AviSynth pixels are square".to_string());
    lines.push(String::new());
    if !is_hdr {
        lines.push("# Output as YUV 4:2:0 (YV12), matching ffmpeg's yuv420p".to_string());
        lines.push("ConvertToYV12()".to_string());
    }
    lines.join("\n")
}
...
/// Parse a tempo value the way ffmpeg's atempo does (av_strtod): plain
/// decimals (`1.5`) or fractions (`3/2`). Returns None for anything else.
...
struct AudioRequest<'a> {
    ...
    sample: Option<u32>,
    /// Playback speed (1.0 = unchanged), same semantics as ffmpeg's atempo.
    tempo: f64,
    normalize: bool,
    loudnorm: bool,
    tracks: &'a [AudioInfo],
...
/// and no bitrate/normalisation/tempo was requested, the stream is copied
/// without re-encoding. `normalize` applies peak normalisation to -1 dB
/// (measured via volumedetect, applied via volume); `loudnorm` applies EBU
/// R128 loudness normalisation (target -16 LUFS); `tempo` applies ffmpeg's
...
fn run_audio_extract(req: AudioRequest) -> ! {
    ...
    if copy {
        ...
        // Collect the audio filters (peak gain / loudnorm / tempo) into a
        // single chain; atempo runs last so the loudness/peak analysis sees
    ...
    if let Some(Err(e)) = child
        .stderr
        .take()
        .map(|stderr| drain_audio_stderr(stderr, progress_dur))
    {
        let _ = child.kill();
...
fn main() {
    ...
    while i < args.len() {
        match args[i].as_str() {
            ...
            "-ap" | "--audiotempo" => {
                let val =
                    require_arg(&args, &mut i, "--audiotempo", "a tempo (e.g. 0.75, 1.5, 2.0, or 3/2)");
                match parse_tempo(&val) {
                    Some(t) if (0.5..=100.0).contains(&t) => audio_tempo = t,
                    _ => fail(&format!(
                        "invalid --audiotempo '{}' (expected 0.5-100.0, decimal or fraction, same as ffmpeg atempo)",
                        val
                    )),
                }
            }
    ...
    if avscript.is_some() && (outpipe || play || audiofile.is_some()) {
        fail("--avscript cannot be combined with --outpipe/--playpreview/--audiofile");
    }
    ...
    if audio_tempo != 1.0 && audiofile.is_none() {
        fail("--audiotempo requires --audiofile");
    }
    ...
    if rescale.is_some() || letterbox || pillarbox || setfps.is_some() || play || outpipe || recode.is_some() || avscript.is_some() {
        ...
        if let Some(avs_path) = avscript {
            // Standalone (-avs without --recode): write the script and print
            // it for inspection, then exit. With --recode the script is only
            // written (silently) and the encode continues afterwards.
            let script = build_avs_script(&positional[0], rescale, letterbox, pillarbox, retime, &info);
            if let Err(e) = std::fs::write(&avs_path, &script) {
                fail(&format!("failed to write avs script '{avs_path}': {e}"));
            }
            if recode.is_none() {
                println!("{script}");
                eprintln!("avs script written: {avs_path}");
                return;
            }
            eprintln!("avs script written: {avs_path}");
        }
