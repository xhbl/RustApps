...
/// `FFVideoSource` (ffms2, exact rational frame rates), retimed with an exact
...
fn build_avs_script(
    ...
    lines.push("# Requires the ffms2 plugin (FFmpegSource2)".to_string());
    if is_hdr {
        lines.push("# and, for HDR sources: avsresize (z_ConvertFormat) + DGTonemap (DGHable)".to_string());
    }
    lines.push(String::new());
    // Escape backslashes for the AviSynth string literal.
    let path = input.replace('\\', "\\\\");
    lines.push(format!("FFVideoSource(\"{path}\")"));
    lines.push(String::new());
